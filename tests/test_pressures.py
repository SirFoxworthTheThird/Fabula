...
def test_trigger_is_not_satisfied_before_its_condition(scenario):
    ...
    early = scene_state([kitchen_event(scene, 1)], characters, world)
    late = scene_state([kitchen_event(scene, seq) for seq in range(1, 6)], characters, world)
...
def test_fact_unspoken_trigger_closes_once_the_fact_is_said(scenario):
    ...
    quiet = scene_state([kitchen_event(scene, seq) for seq in range(1, 5)], characters, world)
    spoken = scene_state(
        [kitchen_event(scene, seq) for seq in range(1, 4)]
        + [kitchen_event(scene, 4, "You broke the music box, didn't you?")], characters, world
    )
...
def test_character_at_trigger_reads_location_from_the_log(scenario):
    ...
    before = scene_state([kitchen_event(scene, 1)], characters, world)
    arrival = Event(
        id=2,
        scene_id=scene.id,
        seq=2,
    ...
    after = scene_state([kitchen_event(scene, 1), arrival], characters, world)
...
def test_max_fires_and_cooldown_are_derived_from_the_log(scenario):
    ...
    fired_once = scene_state(
        ...
        + [kitchen_event(scene, 5, metadata={"pressure_id": pressure.id})], characters, world
    )
    assert not is_eligible(pressure, fired_once, {})  # still cooling down

    cooled = scene_state(
        fired_once.events + [kitchen_event(scene, seq) for seq in range(6, 12)], characters, world
    )
    assert is_eligible(pressure, cooled, {})

    fired_twice = scene_state(
        cooled.events + [kitchen_event(scene, 12, metadata={"pressure_id": pressure.id})], characters, world
    )
    exhausted = scene_state(
        fired_twice.events + [kitchen_event(scene, seq) for seq in range(13, 25)], characters, world
    )
...
def test_unknown_trigger_key_is_rejected_rather_than_always_eligible(scenario):
    world, characters, scene = scenario
    state = scene_state([kitchen_event(scene, 1)], characters, world)
...
def test_arc_escalates_while_sandbox_waits_for_a_lull(scenario):
    ...
    late = scene_state([kitchen_event(scene, seq) for seq in range(1, 25)], characters, world)
...
def test_arc_pressure_grows_with_elapsed_turns(scenario):
    ...
    early = scene_state([kitchen_event(scene, seq) for seq in range(1, 6)], characters, world)
    later = scene_state([kitchen_event(scene, seq) for seq in range(1, 26)], characters, world)
...
def test_selection_is_deterministic_and_prefers_authored_order_on_ties(scenario):
    ...
    state = scene_state([kitchen_event(scene, seq) for seq in range(1, 6)], characters, world)
...
def test_a_trigger_can_name_several_facts_at_once(scenario):
    ...
    def state_after(*contents):
        ...
        return scene_state(events, characters, world)
...
def test_for_turns_counts_from_the_last_fact_to_land(scenario):
    ...
    state = scene_state(events, characters, world)
...
def test_an_unknown_fact_id_in_a_list_satisfies_nothing(scenario):
    ...
    state = scene_state([kitchen_event(scene, 1, "alpha")], characters, world)
